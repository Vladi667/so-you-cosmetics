    ...
    <div className="min-h-screen bg-mist-white overflow-hidden">
        ...
        <div className="container mx-auto px-6 md:px-12 max-w-5xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 sm:gap-12 md:gap-20 items-center">
            <div className="reveal">
              <h2 className=" font-serif text-2xl sm:text-3xl md:text-5xl lg:text-6xl text-slate-stone mb-4 sm:mb-8 md:mb-12">
                ...
                <div key={idx} className="bg-ivory rounded-3xl p-6 sm:p-10 shadow-xl reveal">
                  <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                    ...
                    <div className="flex-1 text-center sm:text-left">
                      <Lien to={`/workshops/${ws.id}`} className="inline-block">
                      ...
                      <Lien to={`/workshops/${ws.id}`} className="mt-6 ml-3 inline-block px-6 py-2 border border-slate-stone/25 text-slate-stone rounded-full text-xs uppercase tracking-widest hover:bg-slate-stone hover:text-white transition-colors">{t('workshopsPage.detailsLink')}</Lien>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="container mx-auto px-6 md:px-12 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-8 lg:gap-16 max-w-4xl mx-auto">
            {details.map((item, index) => (
              <div
                key={index}
                className="group relative bg-ivory rounded-[16px] sm:rounded-[30px] md:rounded-[40px] p-3 sm:p-8 md:p-12 shadow-[0_15px_50px_rgb(0,0,0,0.05)] hover:shadow-[0_30px_70px_rgb(0,0,0,0.1)] transform hover:-translate-y-4 transition-[opacity,transform,box-shadow] duration-300 reveal overflow-hidden border border-slate-stone/5"
              >
      ...
      {isContactModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-ivory rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-modal-in">
            <div className="p-8">
              ...
              <form onSubmit={handleContactSubmit} className="space-y-4">
                ...
                <div className="pt-2 flex gap-4">
                  <button type="submit" disabled={contactStatus.sending} className="flex-1 py-3 bg-slate-stone text-white font-sans uppercase tracking-[0.2em] text-xs rounded-full hover:bg-slate-stone/90 press">{contactStatus.sending ? t('workshopsPage.sending') : t('workshopsPage.send')}</button>
